...
def compute_precision_recall_and_matching_score(gt: np.array, pred: np.array, prob: np.array,
                                 thresh: float):
    ...
    matching_score = tp / len(gt) # num_correct / num_kps according to SuperGlue's implementation

    #    print(f'total positive: {total_positive}, tp: {tp}, fp: {fp}')
    #    print(f'precision: {precision}, recall: {recall}, matching_score: {matching_score}')
    return {'precision': precision, 'recall': recall, 'matching_score': matching_score}
